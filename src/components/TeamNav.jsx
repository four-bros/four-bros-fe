...
export default function TeamNav() {
  ...
  useEffect(() => {
    const fetchTeams = () => {
      axios
        .get("https://four-bros-be.herokuapp.com/teams")
        .then((res) => {
  ...
  teams.forEach((team) => {
    ...
    if (team.is_user === true) {
      let a = (
        <a className="user_team_link" href={href}>
          {team.team_name}
        </a>
      );
      userTeams.push(a);
      ...
      let option = (
        <a className="selected_cpu_team" href={href}>
          {team.team_name}
        </a>
      );
  ...
  const displayTeams = () => {
    if (!teams) {
      let empty_header = React.createElement(
        "h1",
        { className: "empty_header" },
        "Error, no teams found."
      );
      return empty_header;
    } else {
      let user_team_element = (
        <div className="user_team_container">{userTeams}</div>
      );
      ...
      let cpu_dropdown = (
        <div className="cpu_team_dropdown_container">
          ...
          <div className={dropDownClass}>{cpuTeams}</div>
        </div>
      );
